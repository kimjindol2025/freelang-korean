 ...
 *
 * SQLite database utilities (better-sqlite3 backend)
 */
...
export class SQLiteDatabase {
  ...
  async open(): Promise<void> {
    try {
      ...
      throw new Error(`Failed to open database at ${this.dbPath}: ${String(error)}`);
    }
  }
  ...
  async getDatabaseSize(): Promise<number> {
    ...
    try {
      ...
      throw new Error(`Failed to get database size: ${String(error)}`);
    }
  }
  ...
  async backup(backupPath: string): Promise<void> {
    ...
    try {
      ...
      throw new Error(`Failed to backup database: ${String(error)}`);
    }
  }
