...
export class Compiler {
  ...
  private compileStmt(stmt: Stmt): void {
    switch (stmt.kind) {
      ...
      case "for_c_stmt": return this.compileForCStmt(stmt);
  ...
  private compileForCStmt(stmt: Stmt & { kind: "for_c_stmt" }): void {
    ...
    // init 실행 (Stmt or Expr)
    if (stmt.init) {
      if (typeof (stmt.init as any).kind === 'string' && (stmt.init as any).kind === 'var_decl') {
        // init is a var_decl Stmt
        this.compileStmt(stmt.init as Stmt);
      } else {
        // init is an Expr
        this.compileExpr(stmt.init as Expr);
        // init 결과 버림
        this.chunk.emit(Op.POP, stmt.line);
      }
    }
    ...
    if (stmt.condition) {
      this.compileExpr(stmt.condition);
      this.chunk.emit(Op.JUMP_IF_FALSE, stmt.line);
      exitJump = this.chunk.currentOffset();
      this.chunk.emitI32(0, stmt.line);
    }
