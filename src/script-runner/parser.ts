...
export class Parser {
  ...
  private parseForStmt(): Stmt {
    ...
    // Check if C-style for loop: for var i = 0; i < 10; i++
    if (this.check(TokenType.VAR) || this.check(TokenType.LET) || this.check(TokenType.CONST)) {
      // C-style for loop: for <init>; <cond>; <update> { ... }
      // Parse init without consuming trailing semicolon
      const varKw = this.advance(); // var/let/const
      const mutable = varKw.type === TokenType.VAR;
      const name = this.expectIdent("variable name");
      let type: TypeAnnotation | null = null;
      if (this.check(TokenType.COLON)) {
        this.advance(); // :
      ...
      this.expect(TokenType.EQ, "expected '=' in for loop initializer");
      const initExpr = this.parseExpr(0);
      const init: Stmt = { kind: "var_decl", name, mutable, type, init: initExpr, line: varKw.line, col: varKw.col };

      this.expect(TokenType.SEMICOLON, "expected ';' after for loop initializer");
      const condition = this.parseExpr(0);
      this.expect(TokenType.SEMICOLON, "expected ';' after for loop condition");

      // Parse update expression (may be an assignment like i = i + 1)
      const updateExpr = this.parseExpr(0);
      let update: Expr | null = updateExpr;
      if (this.check(TokenType.EQ)) {
        const eq = this.advance();
        const value = this.parseExpr(0);
        update = { kind: "assign", target: updateExpr, value, line: eq.line, col: eq.col };
      }

      this.expect(TokenType.LBRACE, "expected '{' after for loop update");
      const body = this.parseBlock();
      return { kind: "for_c_stmt", init, condition, update, body, line: kw.line, col: kw.col };
    }
